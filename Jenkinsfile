        ...
        steps {
          script {
            sh '''
              ...
              echo $(docker run -v --rm /home/user/WebGoat/:/scan trufflesecurity/trufflehog:latest filesystem /scan -j)
              echo $(docker run -v --rm /home/user/WebGoat/:/scan trufflesecurity/trufflehog:latest filesystem /scan -j | grep -v "\\.git")
